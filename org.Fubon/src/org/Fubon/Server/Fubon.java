...
//	private Table StockTick;
	...
	public static void main(String[] args) {
		try {
			...
			m_DR=new DataManager();
			//m_DR.getStockCloseData();
...
//		boolean			needUpdate=false;
		...
		shlZ.open();
		shlZ.layout();
		queue = DataManager.queue;  //new Signal_Queue();
		...
		while (!shlZ.isDisposed()) {
			...
			if (DataManager.currentStatus.Changed())
			textMoniter.setText(DataManager.currentStatus.read());
			...
			message=DataManager.getMarketTradingDate();
			...
			*/
			/*while ((tsignal=queue.remove())!=null)
			{
				...
				textMoniter.setText(m_DR.currentStatus.read());
				needUpdate=true;
			}
			if (needUpdate)
			{
	...
	protected void createContents() {
		...
		shlZ.addDisposeListener(new DisposeListener() {
			public void widgetDisposed(DisposeEvent arg0) {
				...
				if (rc == SWT.YES) 
				{
					...
					// Make sure DataManager Exit first
					...
					backup_success=FrameParser.closeDataStore.Backup();
					if (!backup_success) System.err.println("Close Data Back up Error !");
					backup_success=FrameParser.tickDataStore.Backup();
					if (!backup_success) System.err.println("Tick Data Back up Error !");
					backup_success=FrameParser.symbolDataStore.Backup();
					if (!backup_success) System.err.println("Symbol Data Back up Error !");
				}
				System.exit(0);
			}
		});
		...
		ConditionSendbutton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (btnCB_Condition1.getSelection())
				{
					...
					m_DR.setRule1(Condition1_Min, Condition1_Day, Condition1_Percent, Condition1_Filter, true);
				...
				{
					Condition1_Filter=btnCB_Condition1_1.getSelection();
					m_DR.setRule1(0, 0, 0, Condition1_Filter, false);
				}
		...
		StartDataReading.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				DataManager.setReadingMethod(0);
				StartReceiving() ;
			}
		});
		...
		StartDataReadingfromFile.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				DataManager.setReadingMethod(1);
				StartReceiving() ;
			}
		});
...
*/		        
		...
		BeginQuery.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				result=m_DR.Dump(symbol);
				textQueryResult.setText(result);
	/*			//textQueryResult.setText(m_DR.Dump(SymbolText.getText()));
				//設置內容器
				...
				tableViewer.setInput(m_DR.getStockTickListbySymbol(SymbolText.getText()));
...
/*	void DBTest()
	...
	*/
	...
	String getQuoteServerStatus()
	{
		...
		*/
		String result="";
		if (DataManager.Reading_Type==0)
			result="從報價Server讀取 ";
		else
			result="從預錄檔案讀取 ";
		switch (DataManager.DataManager_Status)
		{
		case 0:
			result="等待連接";
			break;
		case 1:
			result=result+"接收中"+DataManager.currentStatusMessage;
			break;
		case -1:
			result="讀取錯誤"+DataManager.currentStatusMessage;
			break;
		default:
			result="錯誤"+DataManager.currentStatusMessage;
			break;
		}
		return result;
	}
	...
	void StartReceiving() 
	{
		if (DataManager.DataManager_Status!=1)
		{
			
			//m_DR=new DataManager();
			//m_DR.CleanMarket();
			DataManager_Thread = new Thread(m_DR);
...
/*	public void dumpQueryResult()
	{
		...
		Tick tTick;
		DayLine tDayLine;
		ts=m_DR.getStockTickListbySymbol(SymbolText.getText());
		dl=m_DR.getStockDayLinebySymbol(SymbolText.getText());
